...
export function setInterceptors(instance: AxiosInstance, interceptors?: InterceptorOptions) {
  ...
  instance.interceptors.response.use(
    async <T>(res: AxiosResponse<T>) => {
      return interceptors?.responseInterceptor ? interceptors.responseInterceptor(res) : res;
    },
    (err) => {
      return interceptors?.responseInterceptorCatch ? interceptors.responseInterceptorCatch(err) : Promise.reject(err);
    }
  );
}
